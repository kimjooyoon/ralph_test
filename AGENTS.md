...
- When BUILDING stagnates (no effective FILE updates), the next iteration runs **DDD Ralph** first (`PROMPT_DDD.md`; markdown-only: plan/specs/docs/ralph/**and** `docs/ddd/**/*.md`), then **PLANNING Ralph** (`PROMPT_PLANNING.md`; same roots except `docs/ddd` is planning-optional but typically already updated). Disable DDD with `RALPH_DISABLE_DDD=1` at the repo harness.
